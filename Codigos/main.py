...
def main():
    if __name__ =='__main__':
        ...
        ## [ Save Model ] ##
        save_plot_model = plot_file + f'{type_model}_{auroral_index}_{today}/'
        save_info_model = model_file + f'{type_model}_{auroral_index}_{today}/'
        save_result_model = result_file + f'{type_model}_{auroral_index}_{today}/'
        ...
        ###### [ Porcentage Set ] ######
        df_train, df_val, df_test = create_set_prediction(df, omni_param, auroral_param, set_split, n_split_train_val_test, n_split_train_val, test_size, val_size, processing_file)
        ...
        ###### [ Shift o Delay ] ######
        omni_train, index_train = shifty(df_train, omni_param, auroral_index, shift_length, type_model)
        omni_val, index_val = shifty(df_val, omni_param, auroral_index, shift_length, type_model)
        omni_test, index_test = shifty(df_test, omni_param, auroral_index, shift_length, type_model)
        print()
        print('---------- [ Dimension Set ] ----------')
        print()
        ...
        ###### [ Test Model ] ######
        df_real_pred, metrics_test = test_model(model, criterion, test_loader, save_result_model, type_model, auroral_index, num_epoch, df_test)
